...
type ParamTypeToValueType<T extends DataSourceParamType | ActionParamType> =
  T extends "textInput" | "select" | "password" | "file"
    ? string
...
interface DataSourceCommonParamConfig<T extends DataSourceParamType>
  extends CommonParamConfig<T> {
  updatable?: boolean;
  rules?: readonly ParamRule[];
}

interface DataSourceTextInputParamConfig
  extends DataSourceCommonParamConfig<"textInput"> {}
interface DataSourceKeyValueInputParamConfig
  extends DataSourceCommonParamConfig<"keyValueInput"> {
  valueType?: "json" | "string";
}
interface DataSourcePasswordInputParamConfig
  extends DataSourceCommonParamConfig<"password"> {}
interface DataSourceNumberInputParamConfig
  extends DataSourceCommonParamConfig<"numberInput"> {}
interface DataSourceCheckboxParamConfig
  extends DataSourceCommonParamConfig<"checkbox"> {}
interface DataSourceGroupTitleParamConfig
  extends DataSourceCommonParamConfig<"groupTitle"> {}
interface DataSourceSelectParamConfig
  extends DataSourceCommonParamConfig<"select"> {
...
interface ActionCommonParamConfig<T extends ActionParamType>
  extends CommonParamConfig<T> {}
interface ActionTextInputParamConfig
  extends ActionCommonParamConfig<"textInput"> {}
interface ActionNumberInputParamConfig
  extends ActionCommonParamConfig<"numberInput"> {}
interface ActionBooleanInputParamConfig
  extends ActionCommonParamConfig<"booleanInput"> {}
interface ActionKeyValueInputParamConfig
  extends ActionCommonParamConfig<"keyValueInput"> {
  valueType?: "json" | "string";
}
...
interface ActionGraphQLParamConfig
  extends ActionCommonParamConfig<"graphqlInput"> {}
interface ActionSelectParamConfig extends ActionCommonParamConfig<"select"> {
  options: readonly ParamOption[];
}

type KeyedConfigToDataType<X extends KeyedParamConfig> =
  X extends KeyedParamConfig<infer C, infer K>
    ? {
        [key in K]: ConfigToType<C>;
      }
    : never;

type ActionConfigToDataType<X extends ActionConfig> = X extends ActionConfig<
  infer C,
  infer K
>
  ? ConfigToType<C> & {
...
type KeyedParamConfig<
  C extends Config = SimpleParamConfig,
  K extends string = string
> = C & {
  key: K;
};

type ParamType<T extends { type: string }> = T extends { type: infer K }
  ? K
  : never;
...
export type ActionArrayParamConfig =
  readonly KeyedParamConfig<ActionParamConfig>[];
...
export interface DataSourcePlugin<AC = any, DSC = any>
...
export type DataSourcePluginFactory<
  ActionDataType = any,
  DataSourceConfigType = any
> = (
  context: PluginContext
